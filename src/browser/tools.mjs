...
class CdpPage {
  ...
  async connect() {
    ...
    this.ws.addEventListener("message", event => { const message = JSON.parse(event.data); if (message.id && this.pending.has(message.id)) { const pending = this.pending.get(message.id); this.pending.delete(message.id); pending.cleanup?.(); message.error ? pending.reject(new Error(message.error.message)) : pending.resolve(message.result); } else if (message.method) { for (const listener of this.listeners.get(message.method) || []) listener(message.params); } });
...
function bindPage(value) { value.page.on("Page.frameNavigated", ({ frame }) => { if (!frame.parentId) clearBrowserRefs(value); }); }
...
const browserNavigate = defineTool({ name: "browser_navigate", label: "Browser navigate", description: "Navigate the current browser page to an HTTP(S) URL without embedded credentials, invalidating prior element refs.", parameters: Type.Object({ sessionId: Type.String(), url: Type.String() }), execute: async (_id, params, signal) => { const value = session(params); const url = allowedNavigation(params.url); const finalUrl = await navigateWithOriginPolicy(value, url, signal); return text({ url: finalUrl, requestedUrl: url }); } });
