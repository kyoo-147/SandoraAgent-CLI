...
const browserAction = (name, description, action, properties) => defineTool({ name, label: name, description, parameters: Type.Object({ sessionId: Type.String(), ...properties }), execute: async (_id, params, signal) => { if (signal?.aborted) throw new Error("Browser action cancelled"); const value = session(params); return text(await action(value.page, params)); } });
...
const browserScreenshot = browserAction("browser_screenshot", "Capture a PNG screenshot, optionally saving it inside the workspace.", async (page, p) => { const result = await page.call("Page.captureScreenshot", { format: "png", fromSurface: true }); if (p.path) await writeFile(workspaceFile(p.cwd, p.path), Buffer.from(result.data, "base64")); return { pngBase64: p.path ? undefined : result.data, path: p.path || null }; }, { path: Type.Optional(Type.String()), cwd: Type.Optional(Type.String()) });
