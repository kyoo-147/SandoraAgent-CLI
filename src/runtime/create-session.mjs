...
import { withRunLifecycle } from "./agent-session.mjs";
...
export async function createSandoraSession({ core = process.env.SANDORA_AGENT_CORE || DEFAULT_AGENT_CORE, customTools = [], ...options } = {}) {
  if (core === "pi") return withRunLifecycle(await createPiAgentSession({ ...options, customTools }));
  if (core === "native") {
    const registry = new NativeToolRegistry().registerAll(customTools);
    return withRunLifecycle(await createNativeAgentSession({ ...options, registry }));
  }
