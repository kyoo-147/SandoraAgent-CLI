...
function assertMergeChecks(pr, allowUnchecked) {
  ...
  const failed = checks.filter(check => (check.status && check.status !== "COMPLETED") || !["SUCCESS", "SKIPPED", "NEUTRAL"].includes(check.conclusion || check.state));
...
async function assertPrCreateReady(head, options) {
  const branch = assertUnprotectedBranch(await currentBranch(options));
  if (branch !== head) throw new Error(`Pull-request head must match current branch: ${branch}`);
  const status = commandOutput(await checked("git", ["status", "--porcelain", "--untracked-files=no"], options));
  if (status) throw new Error("Refusing pull-request creation with tracked working-tree changes");
  const upstream = commandOutput(await checked("git", ["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"], options));
  if (!upstream.endsWith(`/${head}`)) throw new Error(`Current branch upstream does not match PR head: ${upstream}`);
  const unpushed = Number(commandOutput(await checked("git", ["rev-list", "--count", `${upstream}..HEAD`], options)));
  if (!Number.isFinite(unpushed) || unpushed !== 0) throw new Error("Refusing pull-request creation with unpushed commits");
}
...
export function createGitTools({
  ...
  return [
    ...
    defineTool({ name: "github_pr_create", label: "Create GitHub PR", description: "Create a pull request from the current clean, fully pushed feature branch after tests and self-review.", parameters: Type.Object({ head: branchSchema, base: Type.Optional(branchSchema), title: Type.String({ minLength: 1, maxLength: 200 }), body: Type.String({ maxLength: 20_000 }) }), execute: async (_id, p, signal, _update, ctx) => { const options = { cwd: ctx.cwd, signal }; const head = await validBranch(p.head, options); await assertPrCreateReady(head, options); return runGitHub(["pr", "create", "--head", head, "--base", p.base ? await validBranch(p.base, options) : "main", "--title", p.title, "--body", p.body], options); } }),
    defineTool({ name: "github_pr_merge", label: "Merge GitHub PR", description: "Merge an open, non-draft, mergeable pull request only when runtime authority is enabled and checks pass, then verify the merged postcondition. Never force-push.", parameters: Type.Object({ number: Type.Integer({ minimum: 1 }), deleteBranch: Type.Optional(Type.Boolean()) }), execute: async (_id, p, signal, _update, ctx) => { if (!allowPrMerge) throw new Error("Pull-request merge capability is disabled; set SANDORA_ALLOW_PR_MERGE=1 with explicit authority"); const options = { cwd: ctx.cwd, signal }; const state = parseGhJson(await runGitHub(["pr", "view", String(p.number), "--json", "state,isDraft,mergeable,reviewDecision,statusCheckRollup"], options)); assertMergeChecks(state, allowUncheckedPrMerge); const result = await runGitHub(["pr", "merge", String(p.number), "--merge", ...(p.deleteBranch === false ? [] : ["--delete-branch"])], options); const postcondition = parseGhJson(await runGitHub(["pr", "view", String(p.number), "--json", "state,mergedAt,mergeCommit,url"], options)); if (postcondition.state !== "MERGED" && !postcondition.mergedAt) throw new Error("GitHub merge command returned without a verified merged postcondition"); return result; } }),
  ];
}
