...
Selecting the native core does not import the Pi adapter or Pi package at runtime; Pi is loaded lazily only when the explicit/default Pi core is selected. The pinned Pi package remains an installed MVP dependency, so this is runtime isolation rather than completion of the longer-term dependency-removal gate.
...
Before any native or Pi-wrapped tool effect, Sandora validates arguments against a bounded supported JSON Schema subset and rejects unsupported schema constructs before tool advertisement. Native sessions durably record the input hash and correlated `policy.decision` plus `tool.call.approved` or `tool.call.denied` before `tool.call.started`; raw arguments are omitted. Approval-gate ALLOW means only that execution may proceed to the tool's own authority checks, not that every downstream capability has been granted. Cooperative aborts are sealed as cancelled receipts; uncertain crash boundaries remain `UNKNOWN_AFTER_RESTART`.
